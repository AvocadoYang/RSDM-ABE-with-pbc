...
#define ATTRIBUTE_NAME  100
#define ATTRIBUTE_VALUE 100
#define PREDICATECONTAIN 100
#define IdCount 100
#define COLUMN 10
#define REVOKE 99
#define PREDICATE 30
...
KEY attributeKeyGen(int k, int m, int personId, SETUP set)
{
    ...
    // K2 start
    element_init_G1(keys.K_2, pairing);
    ...
    for (int i = 0; i < k; i++)
    {
        element_init_G1(keys.K_x[i], pairing);
    ...
    element_t g_zx;
    element_t mul_tmp;
    element_init_G1(g_zx, pairing);
    element_init_G1(mul_tmp, pairing);
    for (int i = 0; i < m; i++)
    {
        element_pow_zn(g_zx, set.pp.g, set.msk.attribute[i]);
        element_mul(mul_tmp, g_zx, set.pp.Hx[0]);
        element_init_G1(keys.D_x[i], pairing);
...
KEY revokKeyGen(int personId, SETUP set, KEY keys)
{
    ...
    element_init_Zr(b2y, pairing);
    element_mul(b2y, b2, keys.y);
    element_init_G1(g_b2y, pairing);
    element_pow_zn(g_b2y, set.pp.g, b2y);
    element_init_G1(keys.D_0, pairing);
    element_init_G1(g_alpha, pairing);
    element_pow_zn(g_alpha, set.pp.g, set.msk.alpha);
    ...
    element_t g_bId;
    element_t g_bIdh;
    element_init_Zr(bId, pairing);
    element_mul(bId, set.msk.b, set.msk.userId[personId]);
    element_init_G1(g_bId, pairing);
    element_pow_zn(g_bId, set.pp.g, bId);
    element_init_G1(g_bIdh, pairing);
    element_mul(g_bIdh, g_bId, set.pp.h);
    element_init_G1(keys.D_1, pairing);
    element_pow_zn(keys.D_1, g_bIdh, keys.y);
    // // D_1 end

    // //D_2 start
    element_t negativeY;
    element_init_Zr(negativeY, pairing);
    element_neg(negativeY, keys.y);
    element_init_G1(keys.D_2, pairing);
...
CT encrypt(int l, int r, PT msg, AP access_policy, SETUP set)
{
    ...
    // compute C start
    element_t Omega_s;
    element_init_GT(Omega_s, pairing);
